...
public class GeneratedSignatureTests
{
    ...
    [InlineData("def hello_world(a: bytes) -> bytes: \n ...\n", "ReadOnlySpan<byte> HelloWorld(ReadOnlySpan<byte> a)")]
    [InlineData("async def hello_world(a: bytes, b: bool = False, c: float = 0.1) -> None: \n ...\n", "Task<PyObject> HelloWorld(byte[] a, bool b = false, double c = 0.1, CancellationToken cancellationToken = default)")]
    [InlineData("async def hello_world(a: bytes) -> bytes: \n ...\n", "Task<byte[]> HelloWorld(byte[] a, CancellationToken cancellationToken = default)")]
