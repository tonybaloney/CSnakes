...
internal sealed class SmtpEmailSender(SmtpClient client) : IEmailSender<ApplicationUser>
{
    public Task SendConfirmationLinkAsync(ApplicationUser user, string email, string confirmationLink)
    {
        var message = new MailMessage("app@blazorapp.com", email, "Confirmation email", $"Please confirm your account by <a href='{confirmationLink}'>clicking here</a>.")
        {
            IsBodyHtml = true
        };
    ...
    public Task SendPasswordResetCodeAsync(ApplicationUser user, string email, string resetCode)
    {
        var message = new MailMessage("app@blazorapp.com", email, "Reset your password", $"Please reset your password using the following code: {resetCode}");
        return client.SendMailAsync(message);
    }

    public Task SendPasswordResetLinkAsync(ApplicationUser user, string email, string resetLink)
    {
        var message = new MailMessage("app@blazorapp.com", email, "Reset your password", $"Please reset your password by <a href='{resetLink}'>clicking here</a>.")
        {
            IsBodyHtml = true
        };
        return client.SendMailAsync(message);
