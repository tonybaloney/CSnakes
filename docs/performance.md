...
(unlike C#) and a lot of CPU time is spent creating call frames.
...
result is converted back to a Python object.
...
be avoided when possible. We have some performance tricks in CSnakes to intern
...
or a few values. Instead, you should use the `IReadOnlyList` interface to get
the values you need.
...
This means that if you pass a large list or dictionary to a Python function,
CSnakes has to eagerly marshal the entire list or dictionary into .NET types.
